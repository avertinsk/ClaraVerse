...
### OCR документов и семантический поиск (RAG)

Загружайте отсканированные PDF — Docling OCR извлекает текст и Markdown. Документы автоматически разбиваются на чанки, векторизуются через Ollama (nomic-embed-text) и индексируются в векторной БД Qdrant. Задавайте вопросы по документам — Клара находит релевантные фрагменты с помощью семантического поиска, а не простого совпадения ключевых слов.
...
| **OCR документов (Docling)** | Распознавание текста в отсканированных PDF с выводом в Markdown               |
| **Семантический поиск (RAG)** | Загруженные документы автоиндексируются в Qdrant для ИИ-поиска по смыслу   |
...
| **Поиск и RAG** | SearXNG (веб-поиск), Qdrant (векторная БД), Ollama Embeddings (nomic-embed-text) |
